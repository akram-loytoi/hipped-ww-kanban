...
## hipped-ww-kanban specifics

This fork extends `ww-kanban` for Hipped's recruitment platform (endorsements/candidate pipeline), serving three consumer views on one component instance: Admin (full read/write), Company member (read-only), Endorser (drag-scoped to own cards). Backend is Supabase.

Companion element `ww-stack` renders each column and is referenced in `ww-config.js` by a hardcoded WeWeb type ID (`5a88036f-22ea-4f8d-b4a5-bc226ef95061`). If structural changes are needed there (not just new passthrough props), it gets forked separately as `hipped-ww-stack` — check before assuming `ww-kanban`-only changes are sufficient.
